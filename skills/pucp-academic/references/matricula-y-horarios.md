...
- Usa `recommend_course_schedules` para generar alternativas y `evaluate_course_schedule` para una combinación concreta. Comprueba que cada opción contenga sus clases, prácticas, laboratorios y exámenes; si `validationStatus` es `incomplete`, no la presentes como horario sin cruces. Explica cruces, huecos, días, preferencias y evidencia de riesgo. Una vacante nunca está garantizada.
